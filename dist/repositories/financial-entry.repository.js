...
export async function getFixedTotals() {
    ...
    const lastMonths = await db("planning_month")
    ...
    const outcomeRows = await db("financial_entry as fe")
    ...
    const monthlyTotalRows = await db("financial_entry as fe")
        .whereIn("fe.planning_month_id", ids || [])
        .select("fe.planning_month_id")
        .select("fe.type")
        .sum({ amount: "fe.amount" })
    ...
    monthlyTotalRows.forEach((r) => {
        ...
        if (monthsMap[pmId]) {
            if (type === "income")
                monthsMap[pmId].totals.income = amount;
            if (type === "outcome")
                monthsMap[pmId].totals.outcome = amount;
        }
    });
